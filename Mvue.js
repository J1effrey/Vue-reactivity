const compileUtil = {
  ...
  text(node, expr, vm) { //expr:msg // <div v-text='person.name'></div>
    let value;
    if(expr.indexOf('{{') !== -1) {
  ...
  on(node, expr, vm, eventName){
    ...
    // console.log(vm.$options.methods[expr])
    // console.log(expr)
    let fn = vm.$options.methods && vm.$options.methods[expr];
    node.addEventListener(eventName, fn.bind(vm), false);

  },
  bind(node, expr, vm, attrName){ //expr pic attrName src
    // const value = this.getVal(expr, vm)
    node[attrName] = vm.$data[expr]
  },
  // 更新函数
  updater: {
